	...
	return {
		...
		start: function () {
			...
			if (rates === null) {
			    ...
			    // gets rates from URL
				$.ajax({
					url: 'https://openexchangerates.org/api/latest.json?app_id=47e7887e69094d879e3a45bd552f0762',
					method: 'GET',
					dataType: 'json',
		...
		init: function () {
			...
			/* Drop down menu */
			var currencymenu = document.createElement("select");
			currencymenu.setAttribute("id", "currencymenu");
			currencymenu.setAttribute("name", "currencymenu");
			for (var currency in currencies)
			 {
				var optionElement = document.createElement("option");
				optionElement.setAttribute("value", currency);
				optionElement.innerHTML = currency + " (" + currencies[currency] + ")";
				currencymenu.appendChild(optionElement);
			}
			$("#currencylist").append(currencymenu);
		},

		/* Convert from USD to Selected Currency */

		convert: function () {
			// gets currency the user want to convert to. 
			var targetCurrency = $("#currencymenu").find(":selected").val();
			//gets amount and converts it to the currency the user selected.  
		 	var money = $("#value").val();
			console.log(money);
			console.log(targetCurrency);
		  	var convert = money * rates.rates[targetCurrency];
			var convertString = formatter.format(convert);

			$("#output").html(convertString);
